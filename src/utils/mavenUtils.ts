...
export async function rawDependencyTree(pomPath: string): Promise<any> {
    ...
    await executeInBackground(`-N com.github.ferstl:depgraph-maven-plugin:graph -DgraphFormat=text -DshowDuplicates -DshowConflicts -DshowVersions -DshowGroupIds -DshowTypes -DoutputDirectory="${outputDirectory}" -DoutputFileName="${outputFileName}"`, pomPath);
